...
for y=110:155 %y axis pixel range/location of kidney
    for x=80:140 %x axis pixel range/location of kidney
        ...
        if(Ip(x,y) > 100 && Ip(x,y) < 120) %black color rgb values
            rgbImage(x,y,1) = 10;
            rgbImage(x,y,2) = 10;
            rgbImage(x,y,3) = 10;
        end
        
        if(Ip(x,y) > 80 && Ip(x,y) < 100)
            rgbImage(x,y,1) = 0;    %green color rgb values
            rgbImage(x,y,2) = 255;
            rgbImage(x,y,3) = 0;
        end
        
        if(Ip(x,y) > 60 && Ip(x,y) < 80)
            rgbImage(x,y,1) = 0; %blue color rgb values
            rgbImage(x,y,2) = 0;
            rgbImage(x,y,3) = 255;
        end
        
        if(Ip(x,y) > 40 && Ip(x,y) < 60)
            rgbImage(x,y,1) = 255;%yellow/orange color rgb values
            rgbImage(x,y,2) = 255;
            rgbImage(x,y,3) = 0;        
        end
        
        if(Ip(x,y) > 20 && Ip(x,y) < 40)
